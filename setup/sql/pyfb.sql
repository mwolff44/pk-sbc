-- FUNCTION: public.create_cdr2()

-- DROP FUNCTION public.create_cdr();

CREATE FUNCTION public.create_cdr2()
    RETURNS trigger
    LANGUAGE 'plpgsql'
    COST 100
...
END IF;
	...
	UPDATE pyfb_company co SET carrier_balance = co.carrier_balance -o_total_cost FROM pyfb_provider pr WHERE co.id = pr.company_id AND cu.id = NEW.orig_provider AND co.provider_balance_update = True;
	UPDATE pyfb_company co SET carrier_balance = co.carrier_balance -t_total_cost FROM pyfb_provider pr WHERE co.id = pr.company_id AND cu.id = NEW.term_provider AND co.provider_balance_update = True;
	
	RETURN NULL;
END;
$BODY$;

ALTER FUNCTION public.create_cdr2()
