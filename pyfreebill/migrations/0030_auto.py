...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding index on 'CustomerRates', fields ['destination']
        db.create_index('customer_rates', ['destination'])

        # Adding index on 'CustomerRates', fields ['prefix']
        db.create_index('customer_rates', ['prefix'])

        # Adding index on 'ProviderRates', fields ['digits']
        db.create_index('provider_rates', ['digits'])

        # Adding index on 'ProviderRates', fields ['destination']
        db.create_index('provider_rates', ['destination'])


    def backwards(self, orm):
        # Removing index on 'ProviderRates', fields ['destination']
        db.delete_index('provider_rates', ['destination'])

        # Removing index on 'ProviderRates', fields ['digits']
        db.delete_index('provider_rates', ['digits'])

        # Removing index on 'CustomerRates', fields ['prefix']
        db.delete_index('customer_rates', ['prefix'])

        # Removing index on 'CustomerRates', fields ['destination']
        db.delete_index('customer_rates', ['destination'])
